...
export default function HomeScreen() {
  ...
  const { activeSessionId, refresh } = useSession();
  ...
  const handleStart = async (day: DayWithCount) => {
    if (activeSessionId) {
      Alert.alert(
        'Sesión en curso',
        'Ya tienes un entrenamiento activo. Termínalo o continúalo antes de empezar otro.'
      );
      return;
    }
  ...
  return (
    <Screen>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        {activeSessionId ? (
          <Pressable style={styles.activeBanner} onPress={() => router.push('/session')}>
            <MaterialCommunityIcons name="play-circle" size={26} color={GymTheme.active} />
            <View style={{ flex: 1 }}>
              <Text style={styles.activeTitle}>Entrenamiento en curso</Text>
              <Text style={styles.activeSub}>Toca para continuar</Text>
            </View>
            <MaterialCommunityIcons name="chevron-right" size={24} color={GymTheme.active} />
          </Pressable>
        ) : null}
...
const styles = StyleSheet.create({
  ...
  activeTitle: { color: GymTheme.active, fontSize: 16, fontWeight: '700' },
  activeSub: { color: GymTheme.textMuted, fontSize: 13 },
  sectionLabel: {
